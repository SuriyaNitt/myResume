...
\begin{tightemize}
\item Accelerated multiple AV modules using CUDA anywhere between 2x and 5x
\item Produced and deployed DNN models for lidar based obstacle detection
\item Working on DNN models for hazards detection through sensor fusion
...
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Accelerated hazards detection code using CUDA achieving 18x speedup and can run under 3ms per LIDAR frame
\end{tightemize}
\sectionsep
...
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Prototyped, designed and implemented hazards detection algorithm for small obstacles using LIDAR
\end{tightemize}
\sectionsep
